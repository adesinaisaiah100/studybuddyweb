...
export default function KeyBenefitsSection() {
  return (
    <section className="w-full bg-[#0B1510] text-white py-16 sm:py-24 md:py-32 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 relative z-10 text-center">
        ...
        <motion.div 
          ...
          className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 md:gap-8 ${outfit.className}`}
        >
          {benefits.map((benefit, index) => (
            <motion.div 
              key={index} 
              variants={itemVariants}
              className="group relative p-6 sm:p-8 rounded-3xl bg-white/[0.03] border border-white/[0.05] hover:bg-white/[0.06] hover:border-green-500/30 transition-all duration-300 flex flex-col items-center text-center"
            >
              {/* Icon Container */}
